...
window.addEventListener('scroll', () => {
  ...
  if (currentScrollY > lastScrollY && currentScrollY > 80) {
    ...
    // Scroll nach oben → Header anzeigen
...
function scaleAppToViewport() {
  ...
  let scale = Math.min(1, viewportWidth / designWidth);
...
window.addEventListener('load', scaleAppToViewport);
window.addEventListener('resize', scaleAppToViewport);
window.addEventListener('orientationchange', scaleAppToViewport);
setTimeout(scaleAppToViewport, 100);